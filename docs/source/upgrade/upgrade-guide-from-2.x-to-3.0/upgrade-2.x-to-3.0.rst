=======================================================
Scylla Manager Upgrade - Scylla Manager 2.x to 3.0
=======================================================

This document describes upgrade guide between the *Major* releases of Scylla Manager 2.x to 3.0
...
This guide covers upgrading Scylla Manager version 2.x to version 3.0, on the following platforms:
...
- Stop the Scylla Manager Server 2.x
- Stop the Scylla Manager Agent 2.x on all nodes
- Upgrade the Scylla Manager Server and Client to 3.0
- Upgrade the Scylla Manager Agent to 3.0 on all nodes
- Run `scyllamgr_agent_setup` script on all nodes
- Reconcile configuration files
- Start the Scylla Manager Agent 3.0 on all nodes
- Start the Scylla Manager Server 3.0
...
**On the Manager Server** display the contents of the scylla-manager.repo and confirm the version displayed is the version 3.0.
...
Upgrade the Scylla Manager Agent to 3.- on all nodes
------------------------------------------------------
...
.. note:: Rolling back to 2.x is not recommended because 3.0 bug fixes and performance optimizations so you will be going back to a lesser version. This should be only used as a last resort.

Rollback procedure contains the same steps as upgrade but with downgrading the components to older version:

- Stop all Scylla Manager tasks (or wait for them to finish)
- Stop the Scylla Manager Server 3.0
- Stop the Scylla Manager Agent 3.0 on all nodes
- Downgrade the Scylla Manager Server and Client to 2.x
- Downgrade the Scylla Manager Agent to 2.x on all nodes
- Bring back old configuration (if there was conflict)
- Start the Scylla Manager Agent 2.x on all nodes
- Start the Scylla Manager Server 2.x
- Validate status of the cluster
